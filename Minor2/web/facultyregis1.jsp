...
FileInputStream fis=null;
	...
	String q="insert into faculty(id,name,branch,pass) values('"+s12[0]+"','"+s1+"','"+s2+"','"+s4+"')"; 
	st.executeUpdate(q);
if(s5.equals("NULL"))
{
       response.sendRedirect("facultyregis.jsp?key="+s12[0]);
}
else
{
        File image= new File(s5);
pstmt = con.prepareStatement("update faculty set img=? where name='"+s1+"' and id='"+s12[0]+"'");
...
int count = pstmt.executeUpdate();
    ...
    else
    {
        ...
        response.sendRedirect("facultyregis.jsp");
    }
}
    }
catch(Exception ex)
